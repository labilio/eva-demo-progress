(function (root) {
  ...
  root.__evaPatch('general', function (source) {
    ...
    source=root.__evaCut(source,"Ht.length>0&&React.createElement(\"div\",{className:\"loop-idp__section\"},React.createElement(\"div\",{className:\"loop-idp__stitle loop-idp__desc-title\"},React.createElement(\"span\",null,St(\"loop.subIssue.title\"),React.createElement(\"em\",{className:\"loop-idp__count\"},\" \",Oi,\" / \",Ht.length)),!Ct&&React.createElement(Button,{theme:\"borderless\",size:\"small\",icon:React.createElement(Plus$c,{size:14}),\"aria-label\":St(\"loop.subIssue.create\"),onClick:()=>Kt(!0)})),React.createElement(\"div\",{className:\"loop-subissues\"},Ht.map(ki=>{const Wi=ISSUE_STATUS_ICON[ki.status],no=PRIORITY_ICON[ki.priority];return React.createElement(\"div\",{key:ki.id,className:\"loop-subissue\",onClick:Ct?void 0:()=>Ea(ki.id)},React.createElement(Wi,{size:14,strokeWidth:2,style:{color:ISSUE_STATUS_HEX[ki.status]}}),React.createElement(\"span\",{className:\"loop-subissue__id\"},ki.identifier),React.createElement(\"span\",{className:\"loop-subissue__title\"},ki.title),React.createElement(\"span\",{className:\"loop-subissue__spacer\"}),React.createElement(no,{size:14,strokeWidth:2,style:{color:PRIORITY_HEX[ki.priority]}}))})))","window.EvaTaskTree.render({key:rt,currentId:rt,rows:Ct?[xt,...Ht]:issuesOf().map(row=>row.id===xt.id?xt:row),readonly:Ct,onOpen:Ea,onChanged:()=>{ca();ct?.();}},{React:reactExports,CreateIssueModal,projectId:currentSpaceId(),Plus:Plus$c,statusIcons:ISSUE_STATUS_ICON,statusColors:ISSUE_STATUS_HEX,statusLabels:{backlog:\"待规划\",todo:\"待办\",in_progress:\"进行中\",in_review:\"审核中\",done:\"已完成\",blocked:\"受阻\",cancelled:\"已取消\"}})","任务详情多层分解树");
    source=root.__evaCut(source,'no=Ht.find(ls=>ls.id===ki)','no=issuesOf().find(ls=>ls.id===ki)','分解树跨层级定位');
