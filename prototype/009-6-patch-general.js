(function (root) {
  ...
  root.__evaPatch('general', function (source) {
    ...
    function CreateIssueModal(props){
      ...
      return window.EvaLoopTaskCreateUI.render(props,{React:reactExports,Modal,Button,LoopButton,Input:ForwardInput,AutoGrowTextarea,Select,LoopPropertyPill,statusOptions:ISSUE_STATUS_ORDER.map(value=>({value,label:({backlog:"待规划",todo:"待办",in_progress:"进行中",in_review:"审核中",done:"已完成",blocked:"受阻",cancelled:"已取消"})[value],icon:React.createElement(ISSUE_STATUS_ICON[value],{size:14,style:{color:ISSUE_STATUS_HEX[value]}})})),priorityOptions:PRIORITY_ORDER.map(value=>({value,label:({urgent:"紧急",high:"高",medium:"中",low:"低",none:"无"})[value],icon:React.createElement(PRIORITY_ICON[value],{size:14,style:{color:PRIORITY_HEX[value]}})})),icons:{X,ChevronRight,Paperclip:Paperclip$3,Trash2},members:store,HumanIdentity:evaMembers().ui.HumanIdentity,project,getPrefix:()=>project?evaProjectIssuePrefix(project):'',createIssue:payload=>props.canCreate&&!props.canCreate()?Promise.reject(new Error('已失去当前会话或项目的访问权限')):createIssue(payload),uploadAttachment:file=>{
