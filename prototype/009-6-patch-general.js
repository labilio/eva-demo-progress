(function (root) {
  ...
  root.__evaPatch('general', function (source) {
        var projectDirectoryComponentSource = String.raw`EvaPinIcon=createLucideIcon("pin",[["path",{d:"M12 17v5",key:"eva-pin-stem"}],["path",{d:"M5 17h14",key:"eva-pin-base"}],["path",{d:"M15 3.3a1 1 0 0 1 .7 1.7L13 7.7V12l1.8 1.8a1 1 0 0 1-.7 1.7H9.9a1 1 0 0 1-.7-1.7L11 12V7.7L8.3 5A1 1 0 0 1 9 3.3z",key:"eva-pin-body"}]]),EvaPinOffIcon=createLucideIcon("pin-off",[["path",{d:"M12 17v5",key:"eva-pin-off-stem"}],["path",{d:"M7 17h10",key:"eva-pin-off-base"}],["path",{d:"M5 5l14 14",key:"eva-pin-off-slash"}],["path",{d:"M15 3.3a1 1 0 0 1 .7 1.7L14 6.7",key:"eva-pin-off-top"}],["path",{d:"M10.3 10.3 11 9.6V7.7L8.3 5A1 1 0 0 1 9 3.3h1",key:"eva-pin-off-body"}]]),EvaProjectList=Object.assign(({dataSource:evaDataSource=[],renderItem:evaRenderItem,emptyContent:evaEmptyContent,className:evaClassName=""})=>React.createElement("div",{className:evaClassName,role:"list"},evaDataSource.length?evaDataSource.map(evaRenderItem):evaEmptyContent),{Item:({header:evaHeader,main:evaMain,extra:evaExtra,className:evaClassName="",...evaProps})=>React.createElement("div",{...evaProps,className:evaClassName},evaHeader,evaMain,evaExtra)}),EvaProjectDirectory=({spaces:evaSpaces,onEnter:evaEnter,onCreate:evaCreate,onSeed:evaSeed})=>{
          const[evaCreateOpen,setEvaCreateOpen]=reactExports.useState(!1),[evaProjectName,setEvaProjectName]=reactExports.useState(""),[evaSelectedClones,setEvaSelectedClones]=reactExports.useState([]),[evaPinnedIds,setEvaPinnedIds]=reactExports.useState(()=>{try{const evaRaw=localStorage.getItem("eva:pinned-project-ids:v2");if(evaRaw===null)return["drive-design"];const evaSaved=JSON.parse(evaRaw||"[]");return Array.isArray(evaSaved)?evaSaved.slice(0,6):["drive-design"]}catch{return["drive-design"]}});
        ...
        function evaMembers(){return evaMembershipStore||(evaMembershipStore=window.EvaMembership.bootstrap(ORG_PEOPLE,loadSpaces(),CHANNELS_BY_SPACE,ORG_CHANNELS)),evaSharedFileStore||(evaSharedFileStore=window.EvaFileSharing.bootstrap(evaMembershipStore)),evaMembershipComponents||(evaMembershipComponents=window.EvaMembersUI.create({React:reactExports,Button,Select,Modal,Table,Input:ForwardInput,Tag,Checkbox,Radio,Switch,PlusIcon:Plus$c,CloseIcon:X,BackIcon:ArrowLeft$3,MailIcon:Mail$1},evaMembershipStore,evaSharedFileStore)),{store:evaMembershipStore,ui:evaMembershipComponents,files:evaSharedFileStore}}
        ...
        source=root.__evaCut(source,'onCreate:gt=>{const St=newSpace(gt),Ct=[...rt,St];','onCreate:(gt,evaClones)=>{const St=newSpace(gt),Ct=[...rt,St];evaMembers().store.createProject(St.id,St.name,evaMembers().store.snapshot().actorId,evaClones);','项目创建成员事务');
