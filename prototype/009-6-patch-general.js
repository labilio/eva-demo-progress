(function (root) {
  ...
  root.__evaPatch('general', function (source) {
    ...
    source=root.__evaCut(source,'}},[pt,xt]);return React.createElement("div",{className:"collab-frame',
      '}},[pt,xt,rt.id,evaProjectMemberRevision]);reactExports.useEffect(()=>{const Ft=Qt=>{const Nt=Qt.detail||{};Nt.projectId===rt.id&&Nt.tab&&mt(Nt.tab)};return window.addEventListener("eva:open-project-view",Ft),()=>window.removeEventListener("eva:open-project-view",Ft)},[rt.id]);return React.createElement("div",{className:"collab-frame","data-eva-project-id":rt.id,"data-eva-project-tab":pt','项目成员变更刷新内容与批注定位上下文');
