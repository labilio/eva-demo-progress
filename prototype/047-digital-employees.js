...
R.useEffect(()=>{if(view==='create') setType(null);},[view,initialType]);
function save(){run(()=>{
...
return h('div',{className:'eva-digital-center__chat'},h('header',{className:'eva-digital-center__head'},btn('数字员工市场',()=>navigate('/eva-stub/数字员工'),{theme:'borderless',icon:h(icons.ArrowLeft,{size:16})}),h('div',{className:'eva-digital-center__actions'},btn('资料',()=>open('detail',a)),btn(store.hasInTeam(a.id)?'已加入 AI 团队':'加入我的 AI 团队',()=>run(()=>{store.addToTeam(a.id);Toast.success('已加入我的 AI 团队');}),{disabled:store.hasInTeam(a.id)}),btn('放进项目',()=>open('project',a)))),h('div',{className:'eva-msg eva-channel-surface'},h(ChannelsView,{key:id,source,onOpenTask:()=>{}})));
}
