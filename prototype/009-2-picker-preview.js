(function(root){
  ...
  root.EvaPickerPreview={create({React:R,Button,Select,Modal,Input,Tag,Checkbox,Radio},store){
    ...
    const modes={
      invite:{label:'邀请人类',title:'邀请项目成员',hint:'选择要邀请加入供应链运营协同的成员。',kinds:['human'],submit:'发送邀请'},
      create:{label:'建群选成员',title:'新建群聊',hint:'',kinds:['human','clone'],submit:'创建群聊并发送邀请'},
    ...
    function SelectionBody({items,selected,onChange,single,showProjectFilter,humanHint}){
      ...
      return h('div',{className:'eva-picker-body'},showProjectFilter&&h('div',{className:'eva-picker-filter'},h(Select,{'aria-label':'按项目筛选',value:project,onChange:setProject,optionList:[{value:'all',label:'全部可选人员'},{value:'供应链运营协同',label:'供应链运营协同'}]})),
        h('div',{className:'eva-picker-list'},
          ...
          ['human','clone'].map(kind=>{const rows=visible.filter(p=>p.kind===kind);return rows.length>0&&h('section',{key:kind},h('div',{className:'eva-picker-group'},h('strong',null,kind==='human'?'人类':'我的分身'),h('span',null,rows.length),h('small',null,kind==='clone'?'仅可带入自己的分身，确认后直接加入。':humanHint||(single?'仅可选择一位人类，分身不能承担治理职责。':'对方接受邀请后加入。'))),rows.map(p=>h('div',{key:p.id,className:'eva-picker-row'+(selected.includes(p.id)?' is-selected':'')},h(single?Radio:Checkbox,{'aria-label':'选择'+p.name,name:'eva-picker-choice',checked:selected.includes(p.id),disabled:!!p.disabled,onChange:()=>{if(!p.disabled)toggle(p.id);}}),p.kind==='clone'?root.EvaAIIdentity.avatar({name:p.name,sourceName:'Eva',logo:root.__EVA_COLLEAGUE_PORTRAIT,ownerName:store.person(p.ownerId)?.name||'',ownerAvatar:p.ownerId==='u-wangyilin'?root.__EVA_CURRENT_USER_PORTRAIT:root.EvaAvatar.personUri(p.ownerId)},36,h):h('span',{className:'eva-picker-avatar'},h('img',{alt:'',src:p.id==='u-wangyilin'?root.__EVA_CURRENT_USER_PORTRAIT:root.EvaAvatar.personUri(p.id)})),h('span',{className:'eva-picker-person'},h('span',{className:'eva-picker-name'},p.name,p.kind==='clone'&&root.EvaAIIdentity.badge(h)),p.detail&&h('span',{className:'eva-picker-detail'},p.detail)),p.selectionHint&&h('span',{className:'eva-picker-selection-hint'},p.selectionHint))))})));
    }
    function Preview(){
      ...
      const items=[...(config.kinds.includes('human')?humans.filter(p=>mode==='invite'?!s.projects.prod.humans.some(m=>m.id===p.id):s.projects.prod.humans.some(m=>m.id===p.id)):[]),...(config.kinds.includes('clone')?s.clones.filter(p=>p.ownerId==='u-wangyilin').map(p=>({...p,kind:'clone',detail:'',project:'供应链运营协同'})):[])];
      ...
      return h(R.Fragment,null,h(Button,{onClick:()=>setOpen(true)},'查看成员选择器模板'),h(Modal,{className:'eva-members-modal eva-picker-modal',width:640,visible:open,title:config.title,footer:null,onCancel:()=>setOpen(false)},h('div',{className:'eva-picker-heading'},config.hint&&h('p',null,config.hint),mode==='create'&&h(Input,{'aria-label':'群聊名称',value:name,onChange:setName,placeholder:'群聊名称'})),
        h(SelectionBody,{key:mode,items,selected,onChange:ids=>{setSelected(ids);setResult(null);},single:config.single,showProjectFilter:mode==='invite'}),
