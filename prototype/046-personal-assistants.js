...
(function () {
  ...
  window.EvaPersonal = Object.freeze({
    ...
    createFolder: function (name) {
      ...
      if (name === '最近' || folders.some(function (f) { return f.name === name; })) throw new Error('已有同名分组');
    ...
    renameFolder: function (id, name) {
      ...
      if (!f) throw new Error('最近分组不能重命名');
      if (!name || name.length > 60) throw new Error('请输入 1–60 字的分组名称');
      if (name === '最近' || folders.some(function (f) { return f.id !== id && f.name === name; })) throw new Error('已有同名分组');
      f.name = name; persist();
    },
    deleteFolder: function (id) {
      if (!id) throw new Error('最近分组不能删除');
