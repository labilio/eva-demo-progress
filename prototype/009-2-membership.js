(function(root){
  'use strict';
  const projectAgentName=project=>root.EvaAIIdentity?.projectAgentName?.(project)||(project?.name?String(project.name)+' · 项目管家':'项目管家');
  ...
  const projectAgentAppearance=project=>root.EvaAIIdentity?.projectAgentAppearance?.(project)||{name:projectAgentName(project),sourceName:'Eva',avatar:'prototype/assets/project-agent-bot.svg',logo:root.__EVA_COLLEAGUE_PORTRAIT,markerKind:'bot'};
  ...
  function create(seed, persist, resolveProjectInfo){
    ...
    const agentFor=pid=>state.projects[pid]?{id:'project-agent:'+pid,name:projectAgentName(projectInfo(pid)),kind:'project-agent',ai:true,projectId:pid,cloud:true,removable:false,ownership:'project',identityAppearance:projectAgentAppearance(projectInfo(pid))}:null;
    ...
    const projectAgentMessage=(id,message)=>{
      ...
      const agent=projectAgent||{id:'b-eva-octo',name:projectAgentName(context),identityAppearance:{...projectAgentAppearance(),name:projectAgentName(context)}};
      const legacyNames=new Set(projectAgentLegacyNames(context));
    ...
    if(officialDemo&&officialProject&&!saved.seededOfficialCommunityV1){
      ...
      if(!saved.groups[officialDemo.id]){
        ...
        for(const [id,messages] of Object.entries(officialDemo.messages))saved.messages[id]=messages.map((m,index)=>{
          ...
          const sender=senderId==='project-agent:official'?{id:senderId,uid:senderId,name:projectAgentName(saved.projects.official),kind:'project-agent',ai:true,projectId:'official'}:saved.people.find(p=>p.id===senderId)||saved.clones.find(p=>p.id===senderId);
