(function(root){
  ...
  root.EvaMembersUI={create({React:R,Button,Select,Modal,Table,Input,Tag,Checkbox,Radio,Switch,PlusIcon,CloseIcon,BackIcon,useNavigate},store,files){
    ...
    function CloneIdentity({clone,showOwner=false}){return h('span',{className:'eva-members-ai-identity'},root.EvaAIIdentity.avatar({name:clone.name,sourceName:'Eva',avatar:clone.avatar||root.__EVA_COLLEAGUE_PORTRAIT,logo:root.__EVA_COLLEAGUE_PORTRAIT},32,h),h('span',{className:'eva-identity-copy'},h('span',{className:'eva-identity-name-row'},h('span',{className:'eva-identity-name-text'},clone.name),root.EvaAIIdentity.badge(h)),showOwner&&root.EvaAIIdentity.ownerLabel(cards.identityModel.resolve(clone.id),h,'candidate')));}
    ...
    function MentionPicker({scopeId,members:sourceMembers,visible,onClose,onChoose}){
      ...
      return h(Modal,{className:'eva-members-modal',title:'提及成员',visible,onCancel:onClose,footer:null},h('p',{className:'eva-members-muted'},sourceMembers?'选择需要协作的 AI；所有人只通知人类。':'每位成员都可以提及项目 AI；所有人只通知人类。'),h('div',{className:'eva-members-mention-list'},h(Button,{onClick:()=>{onChoose('所有人');onClose();}},'所有人（'+members.filter(m=>m.kind==='human').length+' 位人类）'),members.map(m=>h(Button,{key:m.id,theme:'borderless',onClick:()=>{onChoose(m.name);onClose();}},m.identityAppearance?h('span',{className:'eva-members-human-identity'},root.EvaAIIdentity.avatar(m.identityAppearance,32,h),h('span',{className:'eva-identity-copy'},h('span',{className:'eva-identity-name-row'},h('span',{className:'eva-identity-name-text'},m.name),root.EvaAIIdentity.badge(h)),root.EvaAIIdentity.ownerLabel(cards.identityModel.resolve(m.id||m.uid),h,'candidate'))):['project-agent','employee'].includes(m.kind)?h(ProjectAgentIdentity,{agent:m}):m.kind==='clone'?h(CloneIdentity,{clone:store.clone(m.id),showOwner:true}):h(HumanIdentity,{id:m.id})))));
    }
