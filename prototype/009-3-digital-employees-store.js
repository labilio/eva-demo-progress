(function(root){
  ...
  const sendSession=(id,sessionId,text)=>{
    ...
    session.draft='';session.updatedAt=now();publish();return session.id;
  };
  ...
  root.EvaDigitalEmployeesStore={
    ...
    conversationSource(id,sessionId){
      ...
      return {...source,initialDraft:c.draft,onDraftChange:text=>setSessionDraft(id,targetId,text),onSend:text=>{const sentId=sendSession(id,targetId,text);if(sentId)markSessionRead(id,sentId);return !!sentId;}};
    },
