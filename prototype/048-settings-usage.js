...
return h(R.Fragment,null,h('div',{ref:popupHost,className:'eva-settings-portal'}),h(Modal,{visible,title:'设置',getPopupContainer:()=>popupHost.current,className:'eva-settings-dialog',width:'min(960px, calc(100vw - 64px))',footer:null,onCancel:onClose,closeOnEsc:true,maskClosable:false},h('div',{className:'eva-settings-dialog__layout'},h('nav',{'aria-label':'设置菜单'},menus.map(([id,label])=>h(Button,{key:id,icon:h(icons[id],{size:16,strokeWidth:1.8}),theme:tab===id?'light':'borderless',block:true,onClick:()=>setTab(id),'aria-current':tab===id?'page':undefined},label))),h('div',{className:'eva-settings-dialog__content'},tab==='usage'?h(Usage,{onOpen:(id,kind)=>{onClose();navigate(id==='a_eva'?'/guid':kind==='twin'?'/messages?evaIM=my-ai&evaIdentity='+encodeURIComponent(id):'/messages?evaEmployee='+encodeURIComponent(id));}}):h(R.Suspense,{fallback:h('p',null,'加载中…')},h(pages[tab],{key:tab}))))));
};
}
})(window);
