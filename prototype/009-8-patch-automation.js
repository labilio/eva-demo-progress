...
`;
  ...
  return React.createElement(React.Fragment,null,React.createElement(EvaSharedAutomationPage,{scope:"personal",items:Nt,loading:pt,onCreate:()=>Ct(true),onOpenRun:Mt=>Pt(Mt),notice:"每次触发都会创建一个新的 Session，并在这里保留运行记录。"}),React.createElement(CreateTaskDialog,{visible:St,onClose:()=>Ct(false)}));
        ...
        if (projectAutomationStart < 0 || projectAutomationEnd < 0) throw new Error('Loop 原版项目自动化组件不存在');
        var projectAutomationSource = source.slice(projectAutomationStart, projectAutomationEnd);
        if (!projectAutomationSource.includes('className:"loop-automation-cards"') ||
            !projectAutomationSource.includes('React.createElement(AutopilotDetailPage') ||
            !projectAutomationSource.includes('triggerAutopilot(Vt.id)')) {
          throw new Error('Loop 原版项目自动化合同不完整');
        }
        source = root.__evaCut(
          source,
          'React.createElement(Avatar$2,{size:"extra-extra-small",shape:"square",color:avatarColor(Vt.assignee_name??Vt.assignee_id),src:Vt.assignee_avatar},(Vt.assignee_name??"?").slice(0,1).toUpperCase()),React.createElement("span",{className:"loop-automation-card__assignee-name"},Vt.assignee_name??Vt.assignee_id)',
          'React.createElement(EvaLoopIdentityAvatar,{person:{id:Vt.assignee_id,name:Vt.assignee_name??Vt.assignee_id},size:24}),React.createElement("span",{className:"eva-identity-name-row"},React.createElement("span",{className:"loop-automation-card__assignee-name"},Vt.assignee_name??Vt.assignee_id),(Vt.assignee_id?.startsWith("project-agent:")||evaMembers().store.clone(Vt.assignee_id)||evaMembers().store.employee(Vt.assignee_id))&&window.EvaAIIdentity.badge(React.createElement))',
          'Loop 项目自动化执行人身份'
        );
