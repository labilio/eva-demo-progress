(function(root){
  ...
  function create(membership,seed=[],persist,resetSeed=seed,sharedSpaceSeed=DEFAULT_SHARED_SPACES){
    ...
    const visibleRecord=(item,actorId)=>{
      const value=clone(item),agent=membership.projectAgent?.(item.projectId||item.spaceId);delete value.identity;
      if(agent){const legacyNames=root.EvaAIIdentity?.projectAgentLegacyNames({name:agent.name.replace(/ · 项目管家$/,'')})||['Eva 项目管理专员','Eva 项目助手'];for(const key of ['creator','editor','createdBy','updatedBy'])if(legacyNames.includes(value[key]))value[key]=agent.name;}
    ...
    const api={
      ...
      updateExternalLink(actorId,id,draft={}){
        const item=record(id);if(item.type!=='external_link')fail('当前资源不是外部链接');requireAction('edit-external-link',item.spaceId,actorId);
  ...
  function bootstrap(membership){
    ...
    DEFAULT_RECORDS.filter(item=>item.type==='external_link').forEach(item=>{if(!saved.some(savedItem=>savedItem.id===item.id))saved.push(clone(item));});
