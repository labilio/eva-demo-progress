update public.eva_demo_comments
set kind = 'ready', status = 'approved'
where status = 'ready';
...
alter table public.eva_demo_comments
  ...
  check (status in ('open', 'approved', 'doing'));
...
with check (
  ...
  and kind in ('copy', 'ui', 'rebuild', 'function', 'ready')
  and status = 'open'
);
