...
- AI／Codex 的机器批注入口和参数规范位于 `docs/AI_COMMENTS.md`。接手批注任务前必须阅读该文件；使用 `npm run comments -- add|list|reply|status|delete` 直接操作共享批注，禁止用鼠标自动化模拟填写表单。
- 用户要求“批注／评审但不改代码”时，AI 只能写批注，不得修改产品源码。每条 AI 批注必须带页面路径以及 `selector`、稳定元素 ID 或引用文案之一；创建前先查询该页面，避免重复意见。
- AI 创建的批注默认署名 `Codex`、状态为“待讨论”。AI 不得自行设置“已确认”或“已基本定稿”；只有本轮得到人工明确确认后，才能使用 `--confirmed-by-user` 创建或更新为 `approved` / `ready`。“原型修改中”的存储值仍为 `doing`。
- 原型批注是独立评审工具层，不是 Eva 产品 UI。源码位于 `review/`，数据库迁移位于 `supabase/migrations/`；不得把批注列表塞进 Eva 页面布局，也不得劫持 Eva 原有“反馈问题”入口。
- 页面右上角的轻量“批注”入口只负责打开覆盖式侧栏；侧栏默认隐藏、不得挤压或改写 Eva 布局。侧栏是全局评审台，始终展示整个 Demo 的共享批注并标明所属页面；页面定位点仍只显示当前 hash 对应的批注，跨页定位先导航再聚焦。
- 所有同事访问同一 Vercel 站点时，读写的是同一个 Supabase 项目 `eva-demo-comments`，因此能看到同一页面下的共享批注。人类姓名为选填项，未填写时统一显示“匿名同事”；姓名只用于评审署名，不等同于账号体系。
- 前端只能使用 Supabase publishable key；禁止把 secret/service-role key 写入浏览器代码。公开批注表必须启用 RLS，并只为批注与回复明确开放共享的 `SELECT`、`INSERT`、状态 `UPDATE` 和批注 `DELETE`；不设计管理员权限，所有访问者具有相同批注操作能力，不得把权限扩展到其他表。
