...
- 「我的 AI」位于个人模块并紧跟 Eva 同学；中栏明确分为「AI 团队」和「AI 私聊」。个人助理名称与个人 Eva 共用数据源，AI 私聊使用独立 IM 两人群／子区，不复用本地聊天记录。AI 标紧跟名字，公共头像比例禁止业务规则覆盖。
- 「我的 AI」默认团队动态包含本人及全部可用 AI，不可编辑；自定义 AI 团队保存成员快照，消息、草稿及子区按团队隔离。团队群只响应被明确 @ 的 AI，AI 私聊直接发送即触发当前 AI。
...
- 我的 AI 的 AI 私聊使用 AI 身份名称，不泄露底层两人群或子区名称；AI 团队使用团队名，团队子区使用子区名。
...
- IM 的入口模式由 `evaMessageMode` 决定，消息－关注/最近与个人－我的 AI 各自的数据由 `messageSource(evaMessageMode)` 提供，并统一交给 `ChannelsView`。我的 AI 中栏先区分 AI 团队与 AI 私聊，私聊再按云端分身、个人助理、数字员工分组展示其子区承载的会话；模式变化必须通过组件身份或公共状态控制器重置内部会话状态，不得让上一模式的选中会话残留到下一模式。
