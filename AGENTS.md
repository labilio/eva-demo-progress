...
- 我的 AI 团队包含云端分身、个人助理与数字员工；个人助理名称与个人 Eva 共用数据源，团队侧使用独立 IM 两人群／子区，不复用本地聊天记录。AI 标紧跟名字，公共头像比例禁止业务规则覆盖。
...
- 团队 IM 的入口模式由 `evaMessageMode` 决定，消息－关注/最近与团队－我的 AI 各自的数据由 `messageSource(evaMessageMode)` 提供，并统一交给 `ChannelsView`。My AI 中栏必须按云端分身、个人助理、数字员工分组展示其子区承载的会话，不得暴露群人数、子区图标、群／子区面包屑及群治理控件；会话默认命名并允许重命名。模式变化必须通过组件身份或公共状态控制器重置内部会话状态，不得让上一模式的选中会话残留到下一模式。
...
## 2026-09-08 个人 Eva 与单助理决定

用户最新确认个人与团队均只有一个「Eva 同学」，不再创建或编辑个人助理；个人对话按文件夹组织，“默认”也是完整分类。个人输入框可下拉选择文件夹。旧助理团队会话及草稿迁移保留，云端分身和数字员工保持独立身份。以 [个人 Eva 文件夹与单助理规范](docs/个人Eva文件夹与单助理规范.md) 为准，覆盖前文多个人助理树及创建／编辑助理的旧要求。
