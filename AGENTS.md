...
- 「Agent 创建中心」不作为一级导航展示；其页面与直达路由仅作为「我的 Agent」创建个人助理时复用的内部流程保留。数字员工市场及「我的 Agent」中的新建个人助理入口不受影响。
- 不得通过删除或弱化测试让错误实现通过。用户未禁止测试时必须运行相关自动化和浏览器验收；禁止测试时如实标注未验收，不得声称全部功能正常。

## 产品原型的实现深度与快速交付

- 本仓库是用于产品方案、视觉和交互评审的原型。用户未明确要求真实能力、技术验证、生产实现或外部系统联调时，默认以可评审的视觉与交互闭环为实现深度，不自行扩展为生产级解析、持久化、权限、安全策略、服务端接口或架构重构。
- 上传、导入、下载、同步、外部调用等流程可以使用预置内容和本地演示状态表达关键步骤。演示必须支持评审所需的进入、切换、成功、失败、取消和返回等状态；交付说明须明确哪些是模拟交互，禁止把演示状态描述成真实生产能力。
- 用户明确要求“小改”“快速”“只改 UI／文案／样式”时，先完成最小可评审版本并提供本地预览。不得把不阻塞当前评审的相关问题、数据层完善或工程治理静默并入本轮；发现后记录为后续事项。
- 开工说明必须明确本次属于产品原型、技术验证还是生产实现，并说明保留行为与实现边界。实施中如发现必须扩大范围，先说明原因、增加的工作和对交付时间的影响；用户未确认时维持原定原型深度。
- 可见结果与完整交付分阶段进行：先让用户尽快确认方向，再完成本次改动必要的构建、相关自动化和浏览器验收。不得以等待全量检查、PR 或部署为由延迟已经可用的本地评审版本。
- 只有当前改动直接导致且会阻断页面运行或评审主流程的兼容问题可以随本轮修复；已有但独立的问题不得以“顺手修复”为由扩大范围。
