...
test('没有来源群权限时仍可读项目副本，但系统关联和产生方式不泄露群信息',()=>{const {files}=setup();const id=files.transfer('a','p',{name:'群文件.pdf'},{groupId:'g',groupName:'工作群'});const item=files.list('p','b').find(entry=>entry.id===id);assert.equal(files.sourceLabelFor(item,'b'),'从群聊保存');assert.equal(files.relationsFor(item,'b')[0].label,'来源群聊');assert.equal(files.relationsFor(item,'b')[0].restricted,true);assert.equal(files.relationsFor(item,'a')[0].label,'工作群');});
...
test('详情面板将复制内部链接收纳为文件标题旁的紧凑图标按钮',()=>{const drive=fs.readFileSync(new URL('../prototype/020-mode-layer.js',import.meta.url),'utf8');const project=fs.readFileSync(new URL('../prototype/009-1-project-files-ui.js',import.meta.url),'utf8');const styles=fs.readFileSync(new URL('../prototype/050-file-library.css',import.meta.url),'utf8');assert.match(drive,/class="eva-file-detail__copy-link"[^>]*data-drive-action="copy-link"/);assert.match(project,/className:'eva-file-detail__copy-link'[^}]*aria-label':'复制内部链接'/);assert.doesNotMatch(drive,/class="eva-drive__ghost-button"[^>]*data-drive-action="copy-link"/);assert.doesNotMatch(project,/className:'eva-drive__ghost-button'[^}]*onClick:\(\)=>copyLink/);assert.match(styles,/\.eva-file-detail__copy-link\s*\{[\s\S]*?width:\s*28px;[\s\S]*?height:\s*28px;/);assert.match(styles,/\.eva-file-detail__identity--with-action\s*\{[\s\S]*?grid-template-columns:\s*42px minmax\(0, 1fr\) 28px;/);});
