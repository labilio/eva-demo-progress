...
test('置顶文件优先排序且文件夹位于普通文件之前',()=>{const {members,files,sharing}=setup();const older=files.upload('a','p',{name:'较早置顶.pdf',size:1});const newer=files.upload('a','p',{name:'较新置顶.pdf',size:1});const folderId=files.createFolder('a','p','普通文件夹');const plain=files.upload('a','p',{name:'普通文件.pdf',size:1});const seeded=sharing.create(members,files.snapshot(),undefined,[],[],[{actorId:'a',fileId:older,pinnedAt:'2026-09-08T09:00:00.000Z'},{actorId:'a',fileId:newer,pinnedAt:'2026-09-08T10:00:00.000Z'}]);const order=seeded.sortEntries('a',seeded.list('p','a')).map(item=>item.id);assert.equal(JSON.stringify(order.slice(0,2)),JSON.stringify([newer,older]));assert.ok(order.indexOf(folderId)<order.indexOf(plain));});
...
test('文件夹和回收站条目不能置顶，文件快捷方式可以置顶',()=>{const {files}=setup();const folderId=files.createFolder('a','p','资料夹');assert.throws(()=>files.setPinned('a',folderId,true),/文件夹不支持置顶/);const sourceId=files.upload('a','p',{name:'源文件.pdf',size:1});const shortcutId=files.createShortcut('a',sourceId,'personal:a');files.setPinned('a',shortcutId,true);assert.equal(files.pinnedFiles('a').some(item=>item.id===shortcutId&&item.type==='shortcut'),true);files.trash('a',sourceId);assert.throws(()=>files.setPinned('a',sourceId,true),/回收站中的文件不能置顶/);});
...
test('两个文件入口共用置顶动作，文件库提供跨空间置顶聚合视图',()=>{const drive=fs.readFileSync(new URL('../prototype/020-mode-layer.js',import.meta.url),'utf8');const project=fs.readFileSync(new URL('../prototype/009-1-project-files-ui.js',import.meta.url),'utf8');const styles=fs.readFileSync(new URL('../prototype/050-file-library.css',import.meta.url),'utf8');for(const source of [drive,project]){assert.match(source,/togglePin|togglePinned/);assert.match(source,/aria-pressed/);assert.match(source,/eva-drive__pin-button/);assert.match(source,/取消置顶/);}assert.match(drive,/treeButton\('pinned', '置顶文件', 'pin'/);assert.match(drive,/pinnedFiles\(actor\)/);assert.match(drive,/eva-drive__table--pinned/);assert.match(drive,/打开所在位置/);assert.match(styles,/\.eva-drive__pin-button\.is-pinned/);assert.match(styles,/\.eva-drive__row:focus-within \.eva-drive__pin-button/);});
