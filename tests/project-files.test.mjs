...
test('两个文件入口共用文件与文件夹置顶动作，文件库提供跨空间置顶聚合视图',()=>{const drive=fs.readFileSync(new URL('../prototype/020-mode-layer.js',import.meta.url),'utf8');const project=fs.readFileSync(new URL('../prototype/009-1-project-files-ui.js',import.meta.url),'utf8');const styles=fs.readFileSync(new URL('../prototype/050-file-library.css',import.meta.url),'utf8');for(const source of [drive,project]){assert.match(source,/togglePin|togglePinned/);assert.match(source,/aria-pressed/);assert.match(source,/eva-drive__pin-button/);assert.match(source,/取消置顶/);}assert.match(project,/!trashMode\?h\('button',\{className:'eva-drive__pin-button'/);assert.match(drive,/var pinButton = !isTrash \? '<button class="eva-drive__pin-button/);assert.match(drive,/treeButton\('pinned', '置顶文件', 'pin'/);assert.match(drive,/pinnedFiles\(actor\)/);assert.match(drive,/eva-drive__table--pinned/);assert.match(drive,/openResourceLocation\(resource, true\)/);assert.match(drive,/打开所在位置/);assert.match(styles,/\.eva-drive__pin-button\.is-pinned/);assert.match(styles,/\.eva-drive__row:focus-within \.eva-drive__pin-button/);});
