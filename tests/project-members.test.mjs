...
test('普通成员可提及项目专员，回复使用当前项目元信息',()=>{
 const s=setup();s.createProject('p','项目','a',[]);s.addMember('p','a','b');s.sendMessage('all:p','b','@Eva 项目管理专员 负责人是谁？');
...
test('所有人和项目分身提及提供 Octo 原生 mentions 数据，项目分身不可移除',()=>{
 ...
 s.sendMessage('all:p','a','@Eva 项目管理专员 请介绍');assert.ok(s.messagesFor('all:p','a').at(-2).mentions.some(m=>m.uid===s.projectAgent('p').id));assert.throws(()=>s.remove('p','a',s.projectAgent('p').id),/不可移除/);assert.throws(()=>s.removeClone('p','a',s.projectAgent('p').id),/不可移除/);
});
